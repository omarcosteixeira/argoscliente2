...
app.post('/api/send', (req, res) => {
    ...
    if (instance.sendQueue.length > 5000) {
        return res.status(429).json({ error: "A fila deste bot atingiu o limite." });
    }
...
app.post('/api/reset', (req, res) => {
    ...
    try {
        ...
        console.log(`[SISTEMA] Instância ${cleanNumber} completamente eliminada da memória.`);
        res.json({ success: true, message: `A sessão do bot ${cleanNumber} foi encerrada e apagada com sucesso.` });
    } catch (error) {
